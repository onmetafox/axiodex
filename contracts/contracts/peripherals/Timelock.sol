...
contract Timelock is ITimelock {
    ...
    function initAlpManager() external onlyAdmin {
        IAlpManager _alpManager = IAlpManager(alpManager);

        IMintable alp = IMintable(_alpManager.alp());
        alp.setMinter(alpManager, true);

        IUSDG usdg = IUSDG(_alpManager.usdg());
        usdg.addVault(alpManager);

        IVault vault = _alpManager.vault();
        vault.setManager(alpManager, true);
    }

    function initRewardRouter() external onlyAdmin {
        IRewardRouterV2 _rewardRouter = IRewardRouterV2(rewardRouter);

        IHandlerTarget(_rewardRouter.feeAlpTracker()).setHandler(rewardRouter, true);
        IHandlerTarget(_rewardRouter.stakedAlpTracker()).setHandler(rewardRouter, true);
        IHandlerTarget(alpManager).setHandler(rewardRouter, true);
    }
    ...
    function setShortsTrackerAveragePriceWeight(uint256 _shortsTrackerAveragePriceWeight) external onlyAdmin {
        IAlpManager(alpManager).setShortsTrackerAveragePriceWeight(_shortsTrackerAveragePriceWeight);
    }

    function setAlpCooldownDuration(uint256 _cooldownDuration) external onlyAdmin {
        require(_cooldownDuration < 2 hours, "Timelock: invalid _cooldownDuration");
        IAlpManager(alpManager).setCooldownDuration(_cooldownDuration);
    }
