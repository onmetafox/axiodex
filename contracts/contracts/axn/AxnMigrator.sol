...
import "./interfaces/IAxnIou.sol";
import "./interfaces/IAmmRouter.sol";
import "./interfaces/IAxnMigrator.sol";

contract AxnMigrator is ReentrancyGuard, IAxnMigrator {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "AxnMigrator: forbidden");
        _;
    }

    modifier onlySigner() {
        require(isSigner[msg.sender], "AxnMigrator: forbidden");
        _;
    }
    ...
    function initialize(
        ...
        require(!isInitialized, "AxnMigrator: already initialized");
        require(_whitelistedTokens.length == _iouTokens.length, "AxnMigrator: invalid _iouTokens.length");
        require(_whitelistedTokens.length == _prices.length, "AxnMigrator: invalid _prices.length");
        require(_whitelistedTokens.length == _caps.length, "AxnMigrator: invalid _caps.length");
        require(_lpTokens.length == _lpTokenAs.length, "AxnMigrator: invalid _lpTokenAs.length");
        require(_lpTokens.length == _lpTokenBs.length, "AxnMigrator: invalid _lpTokenBs.length");
    ...
    function migrate(
        ...
        require(isMigrationActive, "AxnMigrator: migration is no longer active");
        require(whitelistedTokens[_token], "AxnMigrator: token not whitelisted");
        require(_tokenAmount > 0, "AxnMigrator: invalid tokenAmount");

        if (hasMaxMigrationLimit) {
            migratedAmounts[msg.sender][_token] = migratedAmounts[msg.sender][_token].add(_tokenAmount);
            require(migratedAmounts[msg.sender][_token] <= maxMigrationAmounts[msg.sender][_token], "AxnMigrator: maxMigrationAmount exceeded");
        }

        uint256 tokenPrice = getTokenPrice(_token);
        uint256 mintAmount = _tokenAmount.mul(tokenPrice).div(axnPrice);
        require(mintAmount > 0, "AxnMigrator: invalid mintAmount");

        tokenAmounts[_token] = tokenAmounts[_token].add(_tokenAmount);
        require(tokenAmounts[_token] < caps[_token], "AxnMigrator: token cap exceeded");
        ...
        if (lpTokens[_token]) {
            ...
            require(tokenA != address(0), "AxnMigrator: invalid tokenA");
            require(tokenB != address(0), "AxnMigrator: invalid tokenB");
        ...
        IAxnIou(iouToken).mint(msg.sender, mintAmount);
    }
    ...
    function signApprove(address _token, address _spender, uint256 _amount, uint256 _nonce) external nonReentrant onlySigner {
        ...
        require(!signedActions[msg.sender][action], "AxnMigrator: already signed");
    ...
    function getTokenPrice(address _token) public view returns (uint256) {
        ...
        require(price != 0, "AxnMigrator: invalid token price");
        return price;
    }

    function getIouToken(address _token) public view returns (address) {
        address iouToken = iouTokens[_token];
        require(iouToken != address(0), "AxnMigrator: invalid iou token");
        return iouToken;
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action], "AxnMigrator: action not signalled");
    }
    ...
    function _validateAuthorization(bytes32 _action) private view {
        ...
        if (count == 0) {
            revert("AxnMigrator: action not authorized");
        }
        require(count >= minAuthorizations, "AxnMigrator: insufficient authorization");
    }

    function _clearAction(bytes32 _action, uint256 _nonce) private {
        require(pendingActions[_action], "AxnMigrator: invalid _action");
