...
export function getPositions(
  ...
  for (let i = 0; i < collateralTokens.length; i++) {
    ...
    if (position.collateral.gt(0)) {
      ...
      if (position.averagePrice?.gt(0) && position.markPrice?.gt(0)) {
...
export const Exchange = forwardRef((props, ref) => {
  ...
  const defaultTokenSelection = useMemo(
    () => ({
      [SWAP]: {
        from: AddressZero,
        to: getTokenBySymbol(defaultCollateralSymbol).address,
      },
      [LONG]: {
        from: AddressZero,
        to: AddressZero,
      ...
      [SHORT]: {
        from: getTokenBySymbol(defaultCollateralSymbol).address,
        to: AddressZero,
      },
    }),
  ...
  useImperativeHandle(ref, () => ({
    ...
    onIncreasePosition(key, account, collateralToken, indexToken, collateralDelta, sizeDelta, isLong, price, fee, e) {
      ...
      const indexTokenItem = getToken(indexToken);
