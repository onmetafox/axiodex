...
import excImg from "img/ic_exchange.svg";
...
export default function SwapBox(props) {
  ...
  const [shortCollateralAddress, setShortCollateralAddress] = useLocalStorageByChainId(
    ...
    getTokenBySymbol(defaultCollateralSymbol).address
  );
  ...
  const fromToken = getToken(fromTokenAddress);
    ...
    if (isLong) {
      ...
      if (fromTokenAddress !== toTokenAddress) {
        ...
        if (
          ...
          return [t`Insufficient liquidit`, ErrorDisplayType.Modal, ErrorCode.Buffer];
        }
  ...
  const createIncreaseOrder = () => {
    ...
    const minOut = 0;
    const indexToken = getToken(indexTokenAddress);
    const successMsg = t`
  ...
  const increasePosition = async () => {
    ...
    const contractAddress = getContract("PositionRouter");
  ...
  const onSwapOptionChange = (opt) => {
    ...
    if (opt === SHORT && infoTokens) {
      const fromToken = getToken(tokenSelection[opt].from);
  ...
  const ERROR_TOOLTIP_MSG = {
    [ErrorCode.PoolExceeded]: t`ALP doesn't accept this amount of ${fromTokenInfo.symbol}.`,
  };
  ...
  return (
    <div className="Exchange-swap-box">
      ...
      <div className="Exchange-swap-type-box">
        <Tab
          icons={SWAP_ICONS}
