...
export default function GlpSwap(props) {
  ...
  const glpRewardRouterAddress = getContract("AlpRewardRouter");
  ...
  let glpBalanceUsd;
  if (glpBalance) {
    glpBalanceUsd = glpBalance.mul(glpPrice).div(expandDecimals(1, TLP_DECIMALS));
  }
  const glpSupplyUsd = glpSupply.mul(glpPrice).div(expandDecimals(1, TLP_DECIMALS));

  let reserveAmountUsd;
  if (reservedAmount) {
    reserveAmountUsd = reservedAmount.mul(glpPrice).div(expandDecimals(1, TLP_DECIMALS));
  }
  ...
  const swapToken = getToken(swapTokenAddress);
  ...
  const glpAmount = parseValue(glpValue, TLP_DECIMALS);

  const needApproval =
    isBuying && swapTokenAddress !== AddressZero && tokenAllowance && swapAmount && swapAmount.gt(tokenAllowance);

  const swapUsdMin = getUsd(swapAmount, swapTokenAddress, false, infoTokens);
  const glpUsdMax = glpAmount && glpPrice ? glpAmount.mul(glpPrice).div(expandDecimals(1, TLP_DECIMALS)) : undefined;
    ...
    const updateSwapAmounts = () => {
      if (anchorOnSwapAmount) {
        ...
        if (isBuying) {
          ...
          const nextValue = formatAmountFree(nextAmount, TLP_DECIMALS, TLP_DECIMALS);
  ...
  const fillMaxAmount = () => {
    ...
    setAnchorOnSwapAmount(false);
    setGlpValue(formatAmountFree(maxSellAmount, TLP_DECIMALS, TLP_DECIMALS));
  };
  ...
  const wrappedTokenSymbol = getWrappedToken().symbol;
  const nativeTokenSymbol = getNativeToken().symbol;
        ...
        <div className="App-card GlpSwap-stats-card">
          ...
          <div className="App-card-title">
            <div className="App-card-title-mark">
              <div className="App-card-title-mark-icon">
                <img width={35} src={alpIcon} alt="ALP" />
          ...
          <div className="App-card-content">
            ...
            <div className="App-card-row">
              ...
              <div className="value">
                {formatAmount(glpBalance, TLP_DECIMALS, 4, true)} ALP ($
          ...
          <div className="App-card-content">
            ...
            <div className="App-card-row">
              ...
              <div className="value">
                <Trans>
                  {formatAmount(glpSupply, TLP_DECIMALS, 4, true)} ALP ($
        ...
        <div className="GlpSwap-box App-box">
          ...
          {!isBuying && (
            <BuyInputSection
              ...
              tokenBalance={`${formatAmount(maxSellAmount, TLP_DECIMALS, 4, true)}`}
              inputValue={glpValue}
              onInputValueChange={onGlpValueChange}
              showMaxButton={glpValue !== formatAmountFree(maxSellAmount, TLP_DECIMALS, TLP_DECIMALS)}
              onClickTopRightLabel={fillMaxAmount}
              onClickMax={fillMaxAmount}
              balance={payBalance}
              defaultTokenName={"ALP"}
            >
              <div className="selected-token">ALP</div>
            </BuyInputSection>
          )}
          ...
          {isBuying && (
            <BuyInputSection
              ...
              topRightLabel={t`Balance:`}
              tokenBalance={`${formatAmount(glpBalance, TLP_DECIMALS, 4, true)}`}
              inputValue={glpValue}
              onInputValueChange={onGlpValueChange}
